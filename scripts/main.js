...
var slidePayment = function slidePayment() {
  var slidePayment = qs('.slide-payment'),
    openSlidePayment = qsAll('.open-slide-payment'),
    closeSlidePayment = slidePayment.querySelector('.slide-payment__close'),
    fsmenu = $('.full-screen-menu');
  openSlidePayment.forEach(function (element) {
    element.addEventListener('click', function () {
      slidePayment.style.cssText = "\n\t\t\t\ttransform: translateX(0);\n\t\t\t\ttransition: transform .5s ease-in-out,\n\t\t\t\t-webkit-transform .5s ease-in-out\n\t\t\t";
      qs('body').classList.add('overflow-hidden');
      if (fsmenu.hasClass('checked-full-screen-menu')) {
        jQuery('.full-screen-menu').removeClass('checked-full-screen-menu');
        jQuery('body').removeClass('overflow-hidden');
  ...
  closeSlidePayment.addEventListener('click', function () {
    slidePayment.style.cssText = "\n\t\t\ttransform: translateX(100%);\n\t\t\ttransition: transform .5s ease-in-out,\n\t\t\t-webkit-transform .5s ease-in-out\n\t\t";
    qs('body').classList.remove('overflow-hidden');
    if (fsmenu.hasClass('checked-full-screen-menu')) {
      jQuery('.full-screen-menu').removeClass('checked-full-screen-menu');
...
var initShortcodesDOMContentLoaded = function initShortcodesDOMContentLoaded() {
  ...
  if (ifItExists('.open-slide-payment')) {
    slidePayment();
  }
};
...
// Check if .fc-trigger-close button exists
var fcTriggerCloseButton = document.querySelector(".fc-trigger-close button");
if (fcTriggerCloseButton) {
  fcTriggerCloseButton.addEventListener("click", function () {
    var fcTrigger = document.querySelector(".fc-trigger");
    var fcButton = document.querySelector(".fc-button");
    if (fcTrigger && fcButton) {
      // Slide and fade out .fc-trigger to the right
      fcTrigger.style.right = "-300px"; // Slide .fc-trigger to the right
      fcTrigger.style.opacity = "0";
      setTimeout(function () {
        // Slide and fade in .fc-button from the right
        fcButton.style.opacity = "1";
      }, 300); // Matches the 0.3s transition duration in the Stylus code
    }
  });
}

// Check if .fc-button button exists
var fcButtonElement = document.querySelector(".fc-button button");
if (fcButtonElement) {
  fcButtonElement.addEventListener("click", function () {
    var fcTrigger = document.querySelector(".fc-trigger");
    var fcButton = document.querySelector(".fc-button");
    if (fcTrigger && fcButton) {
      // Slide and fade out .fc-button to the right
      fcButton.style.opacity = "0";
      setTimeout(function () {
        // Slide and fade in .fc-trigger from the right
        fcTrigger.style.right = "0px"; // Slide .fc-trigger into view
        fcTrigger.style.opacity = "1";
      }, 300); // Matches the 0.3s transition duration in the Stylus code
    }
  });
}